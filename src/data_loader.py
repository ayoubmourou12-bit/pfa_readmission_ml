...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cols_drop = [
        c for c in DROP_COLS if c in df.columns and c not in [DEDUP_COL, "encounter_id"]
    ]
...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["lab_per_day"] = df.get("num_lab_procedures", pd.Series(0, index=df.index)) / df[
        "time_in_hospital"
    ].clip(lower=1)
    ...
    if "insulin" in df.columns:
        df["insulin_change"] = (
            (df["insulin"] == 1) & (df.get("change", 0) == 1)
        ).astype(int)

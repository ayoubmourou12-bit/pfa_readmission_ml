...
def _shap_top3(explainer, X_row: pd.DataFrame, feature_names: list) -> list:
    """Retourne les 3 features ayant le plus fort impact SHAP (positif ou négatif)."""
    try:
        sv = explainer.shap_values(X_row)
        # LightGBM multi-output : prendre classe 1
...
def predict_single(patient_data: dict, artifacts: dict = None) -> dict:
    ...
    df      = _prepare(patient_data, artifacts["features"])
    ...
    risk = ("Élevé"  if proba >= 0.15  else
            "Modéré" if proba >= 0.098 else "Faible")
    ...
    result = {
        ...
        "label":            "Réadmission probable (<30j)" if pred
                            else "Pas de réadmission",
...
def predict_batch(df: pd.DataFrame, artifacts: dict = None) -> pd.DataFrame:
    if artifacts is None:
        artifacts = load_artifacts()
    df_enc = feature_engineering(encode(df.copy()))
    ...
    return pd.DataFrame({
        ...
        "risk_level":  ["Élevé" if p >= 0.15 else
                         "Modéré" if p >= 0.098 else "Faible"
                         for p in probas],
    })
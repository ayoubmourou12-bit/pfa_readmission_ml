...
class PatientInput(BaseModel):
    ...
    time_in_hospital: int = Field(..., ge=1, le=14, description="Durée séjour (jours)")
    num_lab_procedures: int = Field(..., ge=1, le=132, description="Nb procédures lab")
    num_procedures: int = Field(..., ge=0, le=6, description="Nb procédures")
    num_medications: int = Field(..., ge=1, le=79, description="Nb médicaments")
    number_outpatient: int = Field(
        ..., ge=0, description="Visites ambulatoires antérieures"
    )
    number_inpatient: int = Field(..., ge=0, description="Hospitalisations antérieures")
    number_emergency: int = Field(..., ge=0, description="Urgences antérieures")
    number_diagnoses: int = Field(..., ge=1, le=16, description="Nb diagnostics")
    ...
    def validate_med_status(cls, v):
        if v not in VALID_MED_STATUS:
            raise ValueError(
                f"Statut médicament invalide '{v}'." f" Attendu : {VALID_MED_STATUS}"
            )
        return v
